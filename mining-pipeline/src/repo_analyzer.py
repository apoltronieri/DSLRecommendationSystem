    ...
    def enrich_from_json(self, filename):
        ...
        for idx, item in enumerate(data, start=1):
            try:
                ...
                print(
                    f"[DEBUG] ({idx}/{len(data)}) Carregando repo {owner}/{name} (SEM API)"
                )
                ...
                print(
                    f"[DEBUG] {owner}/{name} -> contrib:{repo.contributors_count}, commits:{repo.commits_count}, last:{repo.last_commit_date}"
                )
                ...
                if repo.is_valid():
                    ...
                    print(
                        f"[DEBUG] Repo INVALIDO: {owner}/{name} -> score={repo._latest_score}"
                    )

            except Exception as e:
                print(
                    f"[ERRO] Problema ao processar {item.get('owner')}/{item.get('name')}: {repr(e)}"
                )
                continue
...
if __name__ == "__main__":
    ...
    analyzer.enrich_from_json("found_models.json")
    ...
    for repo in analyzer.repositories:
        print(
            f"- {repo.name} | Owner: {repo.owner} | Framework: {repo.framework} | Commits: {repo.commits_count} | Contributors: {repo.contributors_count}"
        )
